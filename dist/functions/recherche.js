...
exports.handler = async function (event, context) {
  const { date, debug, readings, texts } = event.queryStringParameters;
  const sep = '[- ,/_]';
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        available_parameters:[
          ...
          {readings: 'boolean value. 1 to add the reference of the readings of the day. default: 0'},
          {texts: 'boolean value. 1 to add the full texts of the readings of the day. default: 0'}
        ]
      })
    };
  }
  ...
  if(readings) {
    ...
    var psaume = searchPsaume(psaumeIndex, psaumeData, null, tempsLiturgique.anneeLiturgique, tempsLiturgique.tempsLiturgique, tempsLiturgique.numeroSemaine);
    result['psaumeDeLaSemaine'] = psaume;

    const {bibleIndex, bibleData, lecturesIndex, lecturesData} = await initBibleIndex();
    var lectures = searchLecturesByTempsLiturgique(lecturesIndex, lecturesData, tempsLiturgique);
    result['lectures'] = lectures
    result['lecturesIndex'] = lecturesIndex
  }
  return {
    statusCode: 200,
...
async function initBibleIndex() {
  ...
  return {bibleIndex, bibleData, lecturesIndex, lecturesData}
}

function searchLecturesByTempsLiturgique(lecturesIndex, lecturesData, t) {
